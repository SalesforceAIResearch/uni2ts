...
def train_example_gen_func() -> Generator[dict[str, Any], None, None]:
    for i, (product_id, df) in enumerate(train_df.items()):
        yield {
            ...
            "item_id": f"item_{i}",  # 使用枚举的序号i作为item_id
        }
...
def get_data(file_path1,file_path2):
    ...
    df_sales['item_id'] = df_sales['Client'].astype(str) + '-' + df_sales['Warehouse'].astype(str) + '-' + df_sales['Product'].astype(str)
...
val_dataset = datasets.Dataset.from_generator(
    val_example_gen_func, features=features
)
