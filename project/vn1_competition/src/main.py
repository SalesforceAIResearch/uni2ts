...
def load_context(file_path1,file_path2):
    ...
    df_context['item_id'] = df_context['Client'].astype(str) + '-' + df_context['Warehouse'].astype(str) + '-' + df_context['Product'].astype(str)
...
def predict(df_context,df_future,checkpoint_path):
    ...
    cols = ['Client', 'Warehouse', 'Product'] + [col for col in df.columns if col not in ['Client', 'Warehouse', 'Product']]
    df = df[cols]
    # 将日期列名转换为字符串格式
...
def vn1_competition_evaluation(df):
    ...
    # Load Objective
    objective = pd.read_csv(f"/home/zhangxiaobin/dmwyd/uni2ts/project/vn1_competition/data/phase_2_sales.csv").set_index(["Client", "Warehouse", "Product"])
...
def main():
    ...
    checkpoint_path = project_dir.parent.parent / "outputs/VN1/run1/checkpoints/last.ckpt"
