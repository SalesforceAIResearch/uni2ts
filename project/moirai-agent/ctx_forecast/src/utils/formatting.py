...
def format_time_series_forecasting_json(
        history_values:str, history_start:Union[str, None]=None, history_end:Union[str, None]=None, history_frequency:Union[str, None]=None,
        ...
        future_values:Union[str, None]=None, future_start:Union[str, None]=None, future_end:Union[str, None]=None, 
    ...
    # set flags
    history_time_flag = (history_start is not None and history_end is not None and history_frequency is not None)
    ...
    if future_values is None:
        ...
        hist_list, _ = preprocess_values(hist_list, value_format=value_format, missing_value=missing_value)
        ...
        all_values, _ = preprocess_values(all_values, value_format=value_format, missing_value=missing_value)
    ...
    assert pred_length > 0, "pred_length must be provided if future_values is not provided"
...
def format_time_series_prompt(
    ...
    if functionality == 'forecasting':
        ...
        if future_values is None:
            ...
            response += f"{tag_start_future_values}{future_values}{tag_end_future_values}\n"
...
def preprocess_values(value_str:list[str], value_format:str, missing_value:str)->tuple[list[str], float]:
    """
    Preprocess the values.
    Args:
    ...
    def round_to_relative_precision(values:list[float], max_value:float, relative_precision:int=4) -> list[float]:
        """
    ...
    if value_format == 'relative-precision':
        values_to_proc = round_to_relative_precision(values_to_proc, max_scale, relative_precision=4)
...
def parse_values_from_ts_prompt(ts_str:str, ts_type:str='future', special_tags:dict=DEFAULT_SPECIAL_TAGS) -> list[float]:
    """
...
if __name__ == "__main__":
    ...
    instruct_prompt = "Directly predict future values and do not output any reasoning steps."
