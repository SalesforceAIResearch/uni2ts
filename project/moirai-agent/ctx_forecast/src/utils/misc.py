...
import numpy as np
def compute_nrmae(prediction:list[float], future_values:list[float], history_values:list[float]):
    # get valid values
    ...
    nrmae = np.mean(np.abs(diff)) / np.clip(np.mean(np.abs(history_values[valid_mask_history])), a_min=1, a_max=np.inf)
