...
def plot_a_time_series(
    ...
    if forecast_values is not None and future_values is not None:
        assert len(forecast_values) == len(future_values), "forecast_values and future_values must have the same length"

    # plot the time series
    # plt.figure(figsize=(8, 4))

    history_indices = list(range(len(history_values)))
    plt.plot(history_indices, history_values, color="blue", linestyle='-', label="history")
    ...
    if future_values is not None:
        ...
        future_indices = list(range(future_start_idx, future_start_idx + len(future_values)))
        plt.plot(future_indices, future_values, color="red", linestyle='-', label="future")
    ...
    if forecast_values is not None:
        ...
        forecast_indices = list(range(forecast_start_idx, forecast_start_idx + len(forecast_values)))
        plt.plot(forecast_indices, forecast_values, color="green", linestyle='-', label="forecast")
    ...
    if future_values is not None or forecast_values is not None:
        separation_idx = len(history_values)-0.5  # Position between last history and first future/forecast
        ax = plt.gca()
        ax.axvspan(separation_idx, ax.get_xlim()[1], alpha=0.3, color='gray', zorder=0)
    
    plt.gca().xaxis.set_major_locator(plt.MaxNLocator(integer=True)) # Ensure x-axis shows integer indices
...
def mean_absolute_error(ground_truth:list[int|float], prediction:list[int|float]) -> float:
    """
    ...
    assert len(ground_truth) == len(prediction), "ground_truth and prediction must have the same length"
    ...
    if len(autocorrs) > 0:
        ...
        if len(peaks) > 0:
            ...
            results['seasonality'] = {
                ...
                'strength': 'strong' if np.max(autocorrs[peaks]) > 0.7 else 'moderate' if np.max(autocorrs[peaks]) > 0.4 else 'weak'
            }
        else:
