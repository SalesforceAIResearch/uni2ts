...
async def main():
    ...
    parser.add_argument("--task_file", type=str, default="/fsx/sfr/data/multimodal/hsyan/time_series_analysis/cik/tasks_by_model_capability/instruct_following/data.parquet",)
    parser.add_argument("--input_mode", type=str, default="text+image", help="text, image, text+image")
    parser.add_argument("--config_file", type=str, default="config.json", help="Config file path")
    parser.add_argument("--config_name", type=str, default="CONFIG", help="Config name")
    parser.add_argument("--output_dir", type=str, default="./_tmp_/gpt4o/text+image", help="Output dir path")
    ...
    for i in range(len(test_dst)):
        ...
        queries[i] = {
            'idx': test_dst[i]['idx'],
            'system_prompt': test_dst[i].get('system_prompt', "You are a helpful assistant."),
    ...
    try:
        ...
        for k, query_data in queries.items():
            print(f"Procssing query {k}, time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
