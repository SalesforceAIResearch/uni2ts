...
def stats_between_percentile(all_values, p_low, p_high):
    ...
    all_values_between_percentile = all_values_sorted[np.where((all_values_sorted >= percentile_low) & (all_values_sorted <= percentile_high))]
...
def calculate_metrics(historical: list, future: list, prediction: list, id=None, roi_args=[{}]):
    # check prediction
    ...
    if len(prediction) == 0:
        ...
        if len(prediction) == len(future_seq):
            ...
            if len(prediction) < len(future_seq):
                pred_seq = np.array(prediction + [prediction[-1]] * (len(future_seq) - len(prediction)))
                length_correct = False
            else:
    ...
    for roi_arg in roi_args:
        ...
        nmae = np.mean(np.abs(scaled_diff)) / np.clip(np.max(valid_values) - np.min(valid_values), a_min = 1e-6, a_max = np.inf)
        nrmse = np.sqrt(np.mean(scaled_diff**2)) / np.clip(np.max(valid_values) - np.min(valid_values), a_min = 1e-6, a_max = np.inf)
...
if __name__ == '__main__':
    ...
    for key in tqdm(gen_result_dict):
        ...
        prediction = parse_values_from_string(response, start_tag='\\boxed{', end_tag='}', entry_sep=',')
        ...
        if len(historical) == 0 or len(future) == 0:
            ...
            roi_args = [
                {'roi': roi, 'alpha': 1, 'beta': 1,}, 
                {'roi': roi, 'alpha': 3, 'beta': 1,},
            ]
            out = calculate_metrics(historical, future, prediction, id=key, roi_args=roi_args)
