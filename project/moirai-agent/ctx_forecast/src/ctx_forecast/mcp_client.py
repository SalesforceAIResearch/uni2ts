...
class MCPClient:
    ...
    async def connect_to_servers(self):
        ...
        for server_name, server_config in self.server_configs.items():
            try:
                server_params = StdioServerParameters(command=server_config['command'], args=server_config['args'], env=server_config.get('env', None))
                stdio, write = await self.exit_stack.enter_async_context(stdio_client(server_params))
                session = await self.exit_stack.enter_async_context(ClientSession(stdio, write))
                await session.initialize()
                
                response = await session.list_tools()
                self.servers[server_name] = {'session': session, 'tools': response.tools, 'config': server_config}
                for tool in response.tools:
                    assert tool.name not in self.tool_to_server_map, f"Conflict: Tool {tool.name} already exists in the tool_to_server_map"
        ...
        print(f"🧰 Total {len(all_tools)} tools ENABLED: {[tool['name'] for tool in all_tools]}")
    ...
    async def get_all_tools(self) -> List[Dict[str, Any]]:
        ...
        for server_name, server_info in self.servers.items():
            tools_enabled = self.server_configs[server_name].get("enabled_tools", [tool.name for tool in server_info['tools']])
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        result = await self.servers[target_server]['session'].call_tool(tool_name, arguments)
        return result
    ...
    def stop_criteria(self, reply_content: str, count: int) -> bool:
        ...
        if reply_content is not None:
            parsed_values = parse_values_from_string(reply_content, start_tag='\\boxed{', end_tag='}', entry_sep=',')
        else:
            parsed_values = []
    ...
    async def _single_round_query(self, context: list = [], query: str = None, image_path: str = None, step: int = 0) -> tuple:
        # prepare input
        ...
        _logs.extend([f"<<< step:{step} \n Reasoning: {rsp.summary}.\n" for rsp in reply.output if rsp.type == 'reasoning'])
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                if tool_result.content[0].type == 'text':
                    ...
                    context.append({"type": "function_call_output", "call_id": call_id, "output": tool_result_text})

                elif tool_result.content[0].type == 'image':
                    tool_result_text = tool_result.content[1].text #!!!: mcp-server may return (image, save_path)
                    context.append({"type": "function_call_output", "call_id": call_id, "output": tool_result_text})
                    base64_image = tool_result.content[0].data
                    context.append({"role": "user", "content": [
                            {"type": "input_image", "image_url": f"data:image/jpeg;base64,{base64_image}"},
                            {"type": "input_text", "text": f"Image returned from the tool call: {call_id}"},
                    ]})
                else:
                    raise ValueError(f"Unsupported content type: {tool_result.content[0].type}")
                
                _logs.append(f"<<< step:{step} \n[Calling]: {tool_name} --- [Args]: {tool_args}")
                _logs.append(f"<<< step:{step} \n[Calling]: {tool_name} --- [Return]: {tool_result_text}")

            except Exception as e:
                context.append({"type": "function_call_output", "call_id": call_id, "output": f"Error: tool execution failed --- {e}"})
                _logs.append(f"<<< step:{step} \n[Error]: {tool_name} | {e}.")

        return context, _logs, reply.output_text


    async def make_forecast(self, history_info: str, user_instruct: str, image_path: str = None) -> tuple:
        all_logs = []
        step = 0
        context = []
        query = history_info + "\n" + user_instruct
        try:
            while True:
                step += 1
                context, _logs, reply_text = await self._single_round_query(context, query, image_path, step)
                query, image_path = None, None # query and image_path are encoded into context, no need to input in next round
...
async def main():
    ...
    try:
        ...
        history_info = json.dumps({
            key: TEST_SAMPLE[key] 
            for key in ['history_values', 'history_start', 'history_end', 'history_frequency', 'context_info']
        })
        user_instruct = TEST_SAMPLE['user_instruct']
        parsed_values, all_logs, context, system_prompt = await client.make_forecast(history_info, user_instruct)
        print(f'='*50)
        print(parsed_values)
        print(f'='*50)
        print(all_logs)
